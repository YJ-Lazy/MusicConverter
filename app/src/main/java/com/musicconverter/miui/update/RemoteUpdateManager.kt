...
data class RemoteUpdateInfo(
    ...
    val allowedStayVersionCodes: Set<Int> = emptySet(),
...
object RemoteUpdateManager {
    ...
    fun cachedMandatoryUpdate(context: Context, currentVersionCode: Int): RemoteUpdateInfo? {
        ...
        return RemoteUpdateInfo(
            ...
            allowedStayVersionCodes = emptySet(),
    ...
    fun isMandatory(info: RemoteUpdateInfo, currentVersionCode: Int): Boolean {
        return info.force ||
            ...
            currentVersionCode in info.blockedVersionCodes
    }

    /**
     * 后台可指定某些旧版允许长期停留。
     * 该策略只抑制自动更新弹窗；用户手动“检查更新”仍可看到并主动升级。
     * force / minSupportedVersionCode / blockedVersionCodes 始终拥有更高优先级。
     */
    fun isStayAllowed(info: RemoteUpdateInfo, currentVersionCode: Int): Boolean {
        return !isMandatory(info, currentVersionCode) &&
            currentVersionCode in info.allowedStayVersionCodes
    }
    ...
    private fun parseManifest(json: JSONObject): RemoteUpdateInfo {
        return RemoteUpdateInfo(
            ...
            allowedStayVersionCodes = readVersionCodeSet(json, "allowedStayVersionCodes"),
            blockedVersionCodes = readVersionCodeSet(json, "blockedVersionCodes")
        )
    }


    private fun readVersionCodeSet(json: JSONObject, key: String): Set<Int> {
        val array = json.optJSONArray(key) ?: return emptySet()
        val result = LinkedHashSet<Int>()
        for (i in 0 until array.length()) {
            val code = array.optInt(i, 0)
    ...
    private fun extractManifestFields(value: String): JSONObject? {
        ...
        fun intArrayField(name: String): JSONArray? {
            val raw = Regex(
                """["']?$name["']?\s*:\s*\[([^]]*)]""",
                setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL)
            ...
            val array = JSONArray()
            Regex("""\d+""").findAll(raw).forEach { match ->
                match.value.toIntOrNull()?.takeIf { it > 0 }?.let(array::put)
            }
            return array
        }
        ...
        val json = JSONObject()
            ...
            .put("allowedStayVersionCodes", intArrayField("allowedStayVersionCodes") ?: JSONArray())
            .put("blockedVersionCodes", intArrayField("blockedVersionCodes") ?: JSONArray())
