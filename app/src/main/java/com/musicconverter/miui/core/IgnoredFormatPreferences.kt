...
object IgnoredFormatPreferences {
    ...
    /**
     * Groups keep the dialog readable while still covering every format currently recognized
     * by the batch scanner. MP3 remains the default for backward-compatible behavior.
     */
    ...
    fun shouldIgnore(context: Context, fileName: String): Boolean {
        val ext = AudioFormatDetector.encryptedExtension(fileName)
            ?: AudioFormatDetector.extension(fileName)
