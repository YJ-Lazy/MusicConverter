...
from .qmc import QmcDecoder, is_qmc_filename, parse_footer
...
def pick_decoder(path: Path) -> Decoder:
    ...
    if dec is not None:
        return dec
    # QQ Music may append a normal-looking container suffix to an encrypted
    # filename, e.g. `title.mgg0.flac`. Prefer the inner QMC marker.
    if is_qmc_filename(path):
        return _EXT_INDEX[".mgg0"]
